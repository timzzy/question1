...
/**
 * ***************************************************************************************
 *                           (c) 2014  SAMOLTECH STUDIOS LIMITED                         *
 *                                                                                       *
...
class iam_backup
	{
		 ...
		 */
		function iam_backup( $host = 'localhost', $dbname = 'mysql', $dbuser = 'root', $dbpass = '', $struct_only = false, $output = true, $compress = true, $filename = "", $appname="" )
		{
			$this->output = $output;
		...
		function _backup()
		{
            ...
            $newfile .= "# ".$this->app_name." Database Management" . $this->newline;
			$newfile .= "# Database Backup Class by Ezekiel Aliyu" . $this->newline;
		...
		function _get_def( $tablename )
		{
			...
			while ( $row = @mysql_fetch_array($result) )
			{
				...
				if ( $row["Default"] != "" )
				{
					...
					{   // Modified for default values to have single quote '' Olowoyo Samuel July 6th 2009
